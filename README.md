...
- Attach videos the same way (library or camera; recordings capped at 45 seconds and medium quality, uploads capped at 45MB, to fit comfortably under Supabase's free-tier 50MB limit), played back in-app with native controls; video tiles show a play icon rather than a real frame thumbnail, to avoid re-downloading video data just to render a preview
...
1. **First hypothesis: file size.** Recordings are duration-capped (60s) but library videos aren't, and Supabase Storage buckets default to a small size limit unless raised explicitly. Added a clear client-side size check and a matching server-side bucket limit (`supabase/006_video_size_limit.sql`), so an oversized file now fails loudly with a clear message instead of mysteriously. Ruled out as the cause here once the actual video turned out to be only 6 seconds long — nowhere near either limit — but it's a real, permanent safeguard regardless. (The first version of this check used a 200MB video limit, which turned out to be wrong: Supabase's free tier enforces a hard **50MB project-wide cap** regardless of what a bucket's own limit says. Corrected the client check to 45MB and the recording settings — duration and quality — to reliably stay under it, once that came to light.)
