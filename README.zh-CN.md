...
1. **第一个猜测：文件太大。** 拍摄的视频有时长上限（60 秒），但从相册选择的视频没有这个限制，而 Supabase Storage 的存储桶如果不手动调高，默认会有一个比较小的体积上限。于是加了一道明确的客户端体积检查（照片 20MB / 视频 200MB，在 `lib/entries.ts` 里），并同步调高了存储桶的服务端上限（`supabase/006_video_size_limit.sql`），这样超出限制的文件会立刻给出清晰提示，而不是莫名其妙地失败。后来发现那条测试视频只有 6 秒，远远够不到任何一个上限，这个猜测被排除了——不过这道防护本身是真实有效、值得保留的。
